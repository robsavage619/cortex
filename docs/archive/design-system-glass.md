...
cyan-to-violet accent reserved for primary actions and highlights. Generous whitespace,
...
## Color Palette & Roles
Dark-only. The "Light" column is intentionally identical, because there is no light theme;
...
## Component Stylings
**Button primary**: cyan-to-violet gradient (`linear-gradient(135deg, #22D3EE, #8B5CF6)`),
...
**Conviction meter**: 1 to 5 segmented bar; filled segments use the accent gradient,
...
is a gradient monogram: a 16px-radius square with the cyan-to-violet gradient and the
ticker's first one or two letters in white 600. The portal must look complete with monograms
...
## Do's and Don'ts
DO: use semantic color roles via Tailwind tokens, never hardcoded hex in components.
DO: `tabular-nums` on every figure; right-align numeric table columns.
DO: reserve the accent gradient for primary actions, active nav, and key highlights. Scarcity keeps it premium.
DO: subtle motion only, 150 to 250ms ease-out on hover; a brief fade/slide when data updates (Framer Motion `layout` + opacity).
DO: keep glass cards over the dark canvas so `backdrop-blur` reads.
DON'T: no neon green or red. Keep gains and losses muted.
DON'T: no gradient on body text, borders, or large fills. The accent gradient is for actions and highlights only.
DON'T: no hard drop shadows or skeuomorphic bevels. Depth is blur plus glow.
DON'T: no more than 3 font sizes per screen.
DON'T: no buy/sell language, no "recommended", no signal framing. This is a decision tool.
DON'T: never block the UI on logo.dev. The monogram fallback must render instantly.

## Responsive Behaviour
- Mobile (<640px): single column, full-width glass cards, 16px padding, sidebar collapses to a top hamburger, charts shrink to 240px tall.
- Tablet (640 to 1024px): 2-column card grid, sidebar collapsed to icons or hamburger, 32px padding.
