...
- `primaryDocument` may be `xslF345X06/filename.xml`, which is an XSLT rendering path; strip the
...
- Pre-load primary doc map (503 CIKs x 0.15s ~ 75s), which pays for itself vs 282K filename guesses
...
### Pre-registration threshold: derived per run, BHY (not a hardcoded 3.0)
...
- **Journal 2026-08-10b (fund EXIT restoration):** old -> new: fund 2.48 -> 2.42, composite 1.78 -> **1.65**, L/S 1.39 -> 1.23 gross / 0.96 -> 0.78 net. Congress (2.24) and value (0.13) unchanged to the decimal, which pins the deltas to this fix alone. `_load_fund_events` sized every event off `value`; an EXIT closes the position so `value` is 0, `log1p(0)=0` tripped the `weight <= 0` guard, and **all 71,958 EXIT rows were dropped**. The negative leg was TRIM only. Now sized as `prev_shares x last close <= filing_date` (10-day lookback) from the local price cache; restores 7,309 of 8,100 in-universe EXITs, ~21% more event flow, all sell-side. Full re-measure this run: congress 2.24, fund 2.42, quality 1.10, trend 0.71, mom 0.38, value 0.13, insider -0.35, vol -0.47, activism -1.71. Third defect in a row that lived in what the pipeline discarded rather than in the scorer.
...
- The same journal, rewritten for a reader rather than an agent (summary table, one section per change), is `docs/factor-journal.md`. Update both or neither.
